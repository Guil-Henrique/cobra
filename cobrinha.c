...
int main(){
	...
	printf ("0 - dificuldade facil\n1 - dificuldade normal\n2 - dificuldade dificil");
	scanf ("%d", &op);
...
char *buf; 
    ...
    snprintf(buf, sz+1, "\n nickname: %s  pontos: %d Dificuldade: %d\n", nickname, pontos, op);
   

	
	
	pont_arq = fopen ("rank.txt", "a+");
   
    fwrite(buf, 1,strlen(buf)*sizeof (char) , pont_arq);
    fclose(pont_arq);
